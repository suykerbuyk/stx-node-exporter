...
func writeJSONReportToFile(rpt *StxEncMgrJSON) {
	jsonData, err := json.MarshalIndent(rpt, "", "  ")
	if err != nil {
		panic(err)
	}
	err = ioutil.WriteFile("echo.json", jsonData, 0644)
	if err != nil {
		panic(err)
	}
}
func main() {
	...
	// Open our jsonFile
	jsonFile, err := os.Open("../../api/stx-enc-mgr.json")
